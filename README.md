...
- `/questions` 問題一覧: 第75/76回必須/A/B問題339問の検索、カテゴリ、問題区分フィルタ
...
- `/training/quiz/basic-67-2` 問題演習: 第75/76回必須/A/B問題339問の1問表示、即時解説、ブックマーク、正答率、ローカル保存
...
初期データは `src/data` に配置しています。現在は第75/76回獣医師国家試験必須/A/B問題339問を正式データとして収録しています。Supabase連携に切り替える場合は、`src/lib/data.ts` の取得関数をSupabaseクエリに差し替えるだけでページ構造を維持できます。
